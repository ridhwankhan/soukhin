    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          >
            {/* Header */}
            <div className="flex items-center justify-between p-4 border-b border-[#F5F0E8]">
              <div className="flex items-center gap-2">
                <ShoppingBag className="w-5 h-5 text-[#1B4332]" />
                <h2 className="text-lg font-semibold text-[#2D2D2D]">Shopping Cart</h2>
                <span className="text-sm text-[#666666]">({getItemCount()} items)</span>
              </div>
              <button
                onClick={onClose}
            ...
            <div className="flex-1 overflow-y-auto p-4">
              {items.length === 0 ? (
                ...
                <div className="space-y-4">
                  {items.map((item) => (
                    <div key={`${item.product.id}-${item.selectedSize}-${item.selectedColor}`} className="flex gap-4 p-3 bg-[#F8F6F3] rounded-sm">
                      ...
                      <div className="flex-1 min-w-0">
                        <h3 className="font-medium text-[#2D2D2D] text-sm line-clamp-1">{item.product.name}</h3>
                        ...
                        <div className="flex items-center justify-between mt-2">
                          <div className="flex items-center border border-[#D4C4B5] rounded-sm">
                            <button
                              ...
                              className="p-1.5 hover:bg-[#F5F0E8]"
                            >
                      ...
                      <button
                        ...
                        className="p-2 text-[#666666] hover:text-[#C2704A] transition-colors"
                      >
            ...
            {/* Footer */}
            {items.length > 0 && (
              <div className="border-t border-[#F5F0E8] p-4 space-y-4">
                ...
                <p className="text-xs text-[#666666]">Delivery fee calculated at checkout</p>
